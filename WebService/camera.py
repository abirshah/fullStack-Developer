...
class Video(object):
    ...
    def get_frame(self):
        ...
        boxes_mail_bird, confidences_mail_bird, class_ids_mail_bird = self.pet_detection.getNumbers(self.net_mail_bird, width, height)
        ...
        if len(indexes_coco) > 0:
            for i in indexes_coco.flatten():
                if str(coco_classes[class_ids_coco[i]]) == 'bird' or str(coco_classes[class_ids_coco[i]]) == 'person':
                    self.pet_detection.draw_bounding_boxes(boxes=boxes_coco, index=i, classes=coco_classes, class_ids=class_ids_coco,
                        ...
                        if str(mail_bird_classes[class_ids_mail_bird[j]]) == 'mailing_package':
                            self.notif.send_notification("Mailing Package Detected", self.email,
                                                         "Detect at time: " + datetime.datetime.now().strftime(
                                                             "%Y%m%d-%H%M%S"))
                            self.pet_detection.draw_bounding_boxes(boxes=boxes_mail_bird, index=j, classes=mail_bird_classes,
                ...
                if str(coco_classes[class_ids_coco[i]]) == 'cat' or str(coco_classes[class_ids_coco[i]]) == 'dog':
                    ...
                    boxes_body_parts, confidences_body_parts, class_ids_body_parts = self.pet_detection.getNumbers(net_body_parts,
                                                                                                   width, height)
                    ...
                    if len(indexes_body_parts) > 0:
                        for k in indexes_body_parts.flatten():
                            ...
                            # This stores the size of each bounding box into a dictionary
                            self.pet_detection.addingSizeOfBoundingBoxes(str(body_parts_classes[class_ids_body_parts[k]]), w * h)
                            # This stores the proportions of each bounding box into a dictionary
                            self.pet_detection.addingProportionsOfBoundingBoxes(str(body_parts_classes[class_ids_body_parts[k]]), w, h)
                            center_x = x + w / 2
                            center_y = y + h / 2
                            self.pet_detection.addingCentroid(str(body_parts_classes[class_ids_body_parts[k]]), center_x, center_y)
                            color = colors[k]
                            self.pet_detection.draw_bounding_boxes(boxes=boxes_body_parts, index=k, classes=body_parts_classes,
